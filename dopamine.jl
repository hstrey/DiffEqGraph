...
# ╔═╡ 4e5d89b4-4551-11ec-2ea3-dda717277ba8
begin
...
# ╔═╡ 89265e67-8040-49a7-bb8c-3967987aa62b
begin
	function dopamine3!(du,u,p,t)
		α,β,γ,δ,dt = p
		u1n = u[1] + u[1]*(β - exp(-α*u[2]))*dt
		u2n = u[2] + (γ*u[2] - δ*u[1])*dt
		
		du[1] = u1n
		du[2] = ifelse(u2n <= 0, 0.0,u2n)
	end
end

# ╔═╡ c196ed7b-57d6-45f8-a6d4-8720dd480287
alpha = 1.0

# ╔═╡ add6b37f-7e6f-46fb-8f11-821af00a0b61
md"""
$(@bind beta html"β <input type=range min=0.1 max=0.5 step=1e-2>")
$(@bind delta html"δ <input type=range min=10 max=20 step=1e-2>")
$(@bind gamma html"γ <input type=range min=0.2 max=0.7 step=1e-2>")
"""

# ╔═╡ f77d615b-056a-4506-b27d-5e70421704f1
begin
p3=[alpha,beta,gamma,delta,0.01]
prob3 = DiscreteProblem(dopamine3!,[100.0,3500.0],(0.0,15.0),p3)
p3
end

# ╔═╡ c0c61ab1-4a74-4db3-ac4f-ca554b63c8ad
begin
sol3 = solve(prob3,dt=0.01)
plot(sol3.t,sol3[1,:],label="dopamine",linewidth=4,ylim=(0,300),xlabel="t in weeks",ylabel="conc",legendfontsize=12)
plot!(sol3.t,sol3[2,:]/20,label="L-tyrosine",linewidth=4,ylim=(0,300),xlabel="t in weeks",ylabel="conc",legendfontsize=12)
plot!([0,15],[100,100],label="baseline",linestyle=:dashdot,linecolor=:black,ylim=(0,300),xlabel="t in weeks",ylabel="conc",legendfontsize=12)
end
...
git-tree-sha1 = "2112f8b8000d6142f5def4478a17479b0085b66d"
