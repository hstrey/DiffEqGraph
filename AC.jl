...
begin
	function AC!(du,u,p,t)
		α,β,γ,δ,dt = p
		c = u[1] + α*dt - β*u[1]*u[3]*dt
		ac = u[2] + β*u[1]*u[3]*dt - γ*u[2]
		acp = u[3] + δ*u[2]
		
		du[1] = ifelse(c <= 0, 0.0,c)
		du[2] = ifelse(ac <= 0, 0.0,ac)
		du[3] = acp
	end
end
...
md"""
$(@bind alpha html"α <input type=range min=1 max=1000 step=1e-2>")
$(@bind beta html"β <input type=range min=0.01 max=0.1 step=1e-3>")
$(@bind gamma html"γ <input type=range min=0.001 max=0.01 step=1e-4>")
$(@bind delta html"δ <input type=range min=0.05 max=0.1 step=1e-3>")
"""

# ╔═╡ f77d615b-056a-4506-b27d-5e70421704f1
begin
p3=[alpha,beta,gamma,delta,0.001]
prob3 = DiscreteProblem(AC!,[100.0,50.0,10.0],(0.0,1),p3)
p3
end

# ╔═╡ c0c61ab1-4a74-4db3-ac4f-ca554b63c8ad
begin
sol3 = solve(prob3,dt=0.001)
plot(sol3.t*15,sol3[1,:],label="choline",xlabel = "t in weeks",ylabel="conc",ylims = (0,105),linewidth = 4,legendfontsize=12)
plot!(sol3.t*15,sol3[2,:],label="acetylcholine",linewidth = 4,legendfontsize=12)
plot!([0,15],[50,50],linestyle = :dashdot ,linecolor = :black,label="baseline",legendfontsize=12)
end
