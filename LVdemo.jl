...
# ╔═╡ f00c4b2e-ccce-454d-b3ef-6ca9f579d419
begin
...
# ╔═╡ 518ae65a-410e-11ec-341d-8167d31da4ca
md"""
...
# ╔═╡ da8685dd-0ade-4bb0-808d-5ff23a75821b
...
# ╔═╡ b4be3a8f-230f-4c11-b432-6ee846b958a5
begin
	tspan = (0.0,10.0)
	u0 = [1.0,1.0]
end

# ╔═╡ 5876829f-b75b-42e7-9b18-f35f8ed94b22
@bind alpha html"α <input type=range min=1.0 max=2.0 step=1e-2>"

# ╔═╡ 2f66c19e-e157-45fb-96e8-0c0970f7b44d
@bind beta html"β <input type=range min=0.5 max=1.0 step=1e-2>"

# ╔═╡ 0ce76d90-ae1e-4291-a2bb-5c167c1cd806
@bind delta html"δ <input type=range min=2.0 max=4.0 step=1e-2>"

# ╔═╡ ee853968-6562-4201-94e3-5469537ef362
@bind gamma html"γ <input type=range min=0.5.0 max=1.5 step=1e-2>"

# ╔═╡ 0e451c0e-736d-40e4-8e52-bf3c63530217
begin
...
# ╔═╡ 1da01374-801d-4d14-bd96-88565e3a53fd
begin
	sol = solve(prob2,Tsit5())
	println("solving...")
end

# ╔═╡ 9d7d9444-c890-469f-afc7-dffa325b3f81
begin
lv_plot = plot(sol,size=(3000,2000),thickness_scaling=4,label=["x" "y"])
savefig("LV.png")
lv_plot
end

# ╔═╡ 0fc5c080-d576-48e6-868c-a03b83cbdaff
begin
	p1 = [1.33,0.68,2.72,0.8]
	prob1 = ODEProblem(lotka_volterra,u0,tspan,p1)
end

# ╔═╡ 9bf52282-4338-4e13-80ec-5352fd113036
begin
	sol1 = solve(prob1,Tsit5(),saveat=0.1)
	odedata = Array(sol1) + 0.8 * randn(size(Array(sol1)))
	lv_noise = plot(sol1, alpha = 0.3, legend = false); scatter!(sol1.t, odedata')
	savefig("LV_noise.png")
	lv_noise
end

# ╔═╡ 1a166489-6eca-44db-9552-e2c8fcc6cf52
p

# ╔═╡ e956265d-f82f-46fb-8800-e3cecc5d42e3
begin
	...
	chain = sample(model, NUTS(.65),3000)
end

# ╔═╡ de9d1543-f1ac-4375-bc99-e1189d4b1efc
p

# ╔═╡ b167dc1e-4dc2-4a55-848d-eae8a5835088
begin
	summary,quant = describe(chain)
	summary
end

# ╔═╡ ad5c0616-e2b9-4dfe-afbd-130d1ed7e573
begin
p_mcmc = plot(chain)
savefig("mcmc.png")
p_mcmc
...
# ╔═╡ d624ec90-e8f7-48e6-8121-54083c944271
chain_array = Array(chain)

# ╔═╡ 1d93fa5b-27e6-446c-99aa-a85b7d4e2abe
p

# ╔═╡ c44d2947-2db9-49b0-83e5-ed1f2dbd584d
begin
	pl = scatter(sol1.t, odedata')
	for k in 1:300
		rand_index = rand(1:3000)
		p_sample = [chain_array[rand_index,2],
					chain_array[rand_index,3],
					chain_array[rand_index,4],
					chain_array[rand_index,5]]
    	resol = solve(remake(prob1,p=p_sample),Tsit5(),saveat=0.1)
...
# ╔═╡ 77d0e60b-0a2d-49be-af65-a8cbe60ee361
begin
...
# ╔═╡ 39dd0afd-d085-40b3-839e-6e154836d28d
chain2 = sample(model2, NUTS(.65), 5000)

# ╔═╡ 39cc3bb7-033b-4851-8abd-aa80a57cbc53
begin
pl2_mcmc = plot(chain2)
savefig("mcmc2.png")
pl2_mcmc
end

# ╔═╡ 07e6b213-daea-4710-8255-94848dd599c9
begin
	pl2 = scatter(sol1.t, odedata');
	chain_array2 = Array(chain2)
	for k in 1:300
    	resol = solve(remake(prob1,p=chain_array2[rand(1:5000), 2:5]),Tsit5(),saveat=0.1)
